...
@dataclass
class MaxConfig:
    """Configuração centralizada de limites máximos para o otimizador.

    Atributos:
        consecutive_days: Número máximo de dias consecutivos de plantão
            permitido para qualquer profissional. ``None`` desativa o limite.
        hours_per_period: Carga horária máxima acumulada dentro da janela de
            tempo definida por ``period_days``. ``None`` desativa o limite.
        period_days: Duração em dias da janela usada pelo limite de horas.
            Padrão: 30 dias.
    """
    ...
    @classmethod
    def from_config(cls, config: Dict[str, object]) -> "MaxConfig":
        """Constrói ``MaxConfig`` a partir do dicionário de configuração."""
        consecutive_days = config.get("max_consecutive_days")
        if consecutive_days is not None:
            consecutive_days = int(consecutive_days)

        hours_per_period = config.get("max_hours_per_period")
        if hours_per_period is not None:
            hours_per_period = float(hours_per_period)

        period_days = int(config.get("max_hours_period_days", 30))
        return cls(
...
def _can_assign(
    ...
    *,
    ...
    if max_config.consecutive_days is not None and max_config.consecutive_days > 0:
        # Build sorted date list once using set for deduplication, then use
        # bisect for O(log n) insertion instead of sorting a second time.
        dates: List[date] = sorted({s.date for s in current})
        new_date = shift.date
        pos = bisect.bisect_left(dates, new_date)
        if pos >= len(dates) or dates[pos] != new_date:
            dates.insert(pos, new_date)
        if _longest_consecutive_streak(dates) > max_config.consecutive_days:
            return False

    if max_config.hours_per_period is not None:
        period_start = shift.date - timedelta(days=max_config.period_days)
        hours_in_period = sum(
            s.duration.total_seconds() / 3600
            for s in current
            if s.date >= period_start
        )
        if hours_in_period + shift.duration.total_seconds() / 3600 > max_config.hours_per_period:
            return False
...
def optimize_schedule(
    ...
    *,
    ...
    max_config: Optional[MaxConfig] = None,
) -> OptimizationResult:
    ...
    Args:
        ...
        max_config: Limites máximos centralizados. Quando ``None``, os valores
            são lidos automaticamente do dicionário ``config`` via
            :meth:`MaxConfig.from_config`.
    """
    ...
    if max_config is None:
        max_config = MaxConfig.from_config(config)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Sobrepõe max_consecutive_days do arquivo de configuração.",
    )
    parser.add_argument(
        "--max-hours-per-period",
        type=float,
        default=None,
        help="Carga horária máxima por período (em horas). Complementa o arquivo de configuração.",
    )
    parser.add_argument(
        "--max-hours-period-days",
        type=int,
        default=None,
        help="Duração em dias da janela para --max-hours-per-period (padrão: 30).",
    )
    ...
    mx = MaxConfig.from_config(config)
    if args.max_consecutive_days is not None:
        mx = MaxConfig(
            consecutive_days=args.max_consecutive_days,
            hours_per_period=mx.hours_per_period,
            period_days=mx.period_days,
        )
    if args.max_hours_per_period is not None:
        mx = MaxConfig(
            consecutive_days=mx.consecutive_days,
            hours_per_period=args.max_hours_per_period,
            period_days=mx.period_days,
        )
    if args.max_hours_period_days is not None:
        mx = MaxConfig(
            consecutive_days=mx.consecutive_days,
            hours_per_period=mx.hours_per_period,
            period_days=args.max_hours_period_days,
        )
    result = optimize_schedule(config, iterations=args.iterations, seed=args.seed, max_config=mx)
